...
export const UserFollowings = ({ userId }: UserFollowingsProps) => {
	...
	useEffect(() => {
		const fetchFollowings = async () => {
			setLoading(true)
			try {
				const response = await fetch(`/api/follows/following?userId=${userId}`, { method: "GET" })
				if (!response.ok) {
					setUsers([])
					return
				}
	...
	if (users.length === 0) {
		return <p className="px-4 py-6 text-sm text-gray-400">Takip edilen kullanıcı bulunamadı.</p>
	}
