...
export async function GET(request: Request) {
  ...
  const endpoint = userId
    ? `${API_BASE_URL}/auth/follows/followers/?user_id=${userId}`
    : `${API_BASE_URL}/auth/follows/followers/`;
  ...
  if (!response.ok) {
    ...
    return Response.json({ detail: data || "Could not fetch followers list" }, { status: response.status });
  }
